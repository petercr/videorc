...
- 2026-07-08: Windows complete-app parent track added in Obsidian; repo W0/W1
  reconciliation now includes the Windows acceptance template and ignored
  artifact convention.
- 2026-07-08: Plan 019 Step 3 started with Windows ffmpeg input builders,
  selected-source ID parsing, DXGI display discovery, MediaFoundation camera
  and microphone discovery, Windows device-list exposure, and recording
  primary-input layout tests for display, camera, and microphone. Backend Job
  Object wrappers now own capture and media-maintenance FFmpeg/FFprobe children
  on Windows. Follow-up slices added preview source recognition, configured
  FFmpeg path propagation, BGRA preview frame runners, and a local-gate
  acceptance manifest. Recording FFmpeg args now select MediaFoundation
  `h264_mf` on Windows instead of the macOS VideoToolbox literal.
...
1. **Plan 019 - Windows v1 capture/package acceptance**
   - Parent track: Obsidian
     `2026-07-07 - Videorc Complete Windows App Plan`.
   ...
   - W0 evidence convention is done: copy
     ...
     ignored `docs/acceptance/artifacts/windows/<date>/`; optionally set
     `VIDEORC_WINDOWS_ACCEPTANCE_DIR` so `smoke:local-gates:windows` writes
     directly into the matching artifact folder.
...
- **P7-S2.5 Windows evidence base - DONE**: Parent W0 evidence template and
  ...
  note and leave generated proof under `docs/acceptance/artifacts/windows/`.
- **P7-S3 Windows capture MVP - IN PROGRESS**: Plan 019 Step 3. Windows ffmpeg
  input builders, selected-source ID parsing, DXGI display discovery,
  ...
  into the existing frame stores. The Windows local gate writes an acceptance
  manifest for command/error/evidence traceability; dshow symbolic-link
  behavior, first-frame/smoothness proof, on-box recording evidence, and
  process-tree cleanup proof are still pending.
...
- **P7-S5 Signing decision - DONE (status only)**: Plan 019 Step 5. Windows
  builds stay unsigned for internal testing until acceptance passes; public
  distribution remains blocked on later Azure Trusted Signing vs OV/EV
  Authenticode selection.
